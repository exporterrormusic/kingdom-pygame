...
class Bullet:
    ...
    def __init__(self, x: float, y: float, angle: float, bullet_type: BulletType = BulletType.PLAYER, speed: float = 800, damage: int = None, size_multiplier: float = 1.0, color: tuple = None, penetration: int = 1, shape: str = "standard", range_limit: float = None, weapon_type: str = None):
        """Initialize a bullet."""
    ...
    def _render_bullet_content(self, screen: pg.Surface, offset=(0, 0)):
        ...
        if self.type == BulletType.ENEMY_LASER:
            ...
            if self.shape == "pellet":
                ...
                # Military-style contrail for anime tracer rounds
                for i, trail_pos in enumerate(self.trail_positions[:-1]):
                ...
                for i, trail_pos in enumerate(self.trail_positions[:-1]):
                    ...
                    # Neon holographic trail colors (cyan-based)
                    trail_color_glow = (
                        int(0),           # No red
            ...
            if self.shape == "laser":
                ...
                pg.draw.circle(screen, self.color, (int(render_x), int(render_y)), self.size)
                pg.draw.circle(screen, (255, 255, 255), (int(render_x), int(render_y)), self.size, 1)
    ...
    def render_shotgun_pellet(self, screen: pg.Surface, x: float, y: float):
        """Render a balanced anime-style fire orb for shotgun pellets."""
        import math
        import random
        
        # Reasonable base pellet size (scaled back from massive 7.5x)
        base_size = int(self.size * 3.0)  # More reasonable size
        
        # Create dynamic pulsing effect
        pulse_time = pg.time.get_ticks() / 200
        pulse_factor = 1.0 + 0.2 * math.sin(pulse_time + hash((x, y)) % 100)  # Reduced pulsing
        current_size = int(base_size * pulse_factor)
        
        # Scaled down outer flame aura
        outer_flame_size = int(current_size * 2.0)  # Reduced from 4.0x
        outer_colors = [(255, 80, 0, 15), (255, 120, 0, 20), (255, 60, 0, 10)]  # Lower alpha
        for i, color in enumerate(outer_colors):
            flame_size = outer_flame_size - i * 4
            if flame_size > 0:
                self._draw_flame_layer(screen, x, y, flame_size, color, pulse_time)
        
        # Reduced fire sparks around the main orb
        spark_count = 6  # Reduced from 12
        for i in range(spark_count):
            angle = (i / spark_count) * 2 * math.pi + pulse_time * 0.5
            spark_dist = current_size * 1.5 + random.uniform(-5, 5)  # Smaller variation
            spark_x = x + math.cos(angle) * spark_dist
            spark_y = y + math.sin(angle) * spark_dist
            spark_size = random.randint(1, 3)  # Smaller sparks
            spark_color = random.choice([(255, 200, 0), (255, 150, 0), (255, 100, 0)])
            pg.draw.circle(screen, spark_color, (int(spark_x), int(spark_y)), spark_size)
        
        # Balanced fire orb layers
        
        # Outer fire glow (red-orange)
        outer_glow_size = int(current_size * 1.8)  # Reduced from 2.8x
        outer_color = (255, 60, 0, 40)  # Lower alpha
        self._draw_energy_orb_layer(screen, x, y, outer_glow_size, outer_color)
        
        # Middle fire layer (orange)
        mid_glow_size = int(current_size * 1.3)  # Reduced from 2.0x
        mid_color = (255, 120, 0, 70)  # Lower alpha
        self._draw_energy_orb_layer(screen, x, y, mid_glow_size, mid_color)
        
        # Inner flame core (yellow-orange)
        inner_glow_size = int(current_size * 1.0)  # Reduced from 1.4x
        inner_color = (255, 180, 20, 100)  # Lower alpha
        self._draw_energy_orb_layer(screen, x, y, inner_glow_size, inner_color)
        
        # Hot core (reduced size)
        core_size = int(current_size * 0.6)  # Reduced from 0.8x
        core_color = (255, 255, 200)
        pg.draw.circle(screen, core_color, (int(x), int(y)), core_size)
        
        # Small bright center
        spark_core_size = int(current_size * 0.3)  # Reduced from 0.4x
        spark_core_color = (255, 255, 255)
        pg.draw.circle(screen, spark_core_color, (int(x), int(y)), spark_core_size)
    ...
    def render_tracer_round(self, screen: pg.Surface, x: float, y: float):
        ...
        # Outer glow layer (anime-style bright aura)
        outer_glow_width = self.size * 2.5
    ...
    def render_enhanced_minigun_bullet(self, screen: pg.Surface, x: float, y: float):
        """Render sci-fi anime plasma bullet for minigun."""
        import math
        
        # Draw sci-fi energy projectile instead of physical bullet
        angle_rad = math.radians(self.angle)
        
        # Sci-fi energy bolt dimensions - made 2.5x bigger
        bolt_length = self.size * 7.5  # Increased 2.5x from 3.0
        bolt_width = self.size * 2.5   # Increased 2.5x from 1.0
        
        # Create energy bolt surface - bigger
        bolt_surface = pg.Surface((int(bolt_length * 2), int(bolt_width * 4)), pg.SRCALPHA)
        surf_center_x = int(bolt_length)
        surf_center_y = int(bolt_width * 2)
        ...
        # Draw energy bolt with multiple layers - bigger and more visible
        # Outer energy field - bigger
        outer_length = int(bolt_length * 1.2)  # Increased
        outer_width = int(bolt_width * 2)      # Increased
        outer_rect = pg.Rect(surf_center_x - outer_length//2, surf_center_y - outer_width//2, 
                           outer_length, outer_width)
        pg.draw.ellipse(bolt_surface, (0, 150, 255, 120), outer_rect)
        
        # Inner energy core - bigger
        inner_length = int(bolt_length * 0.9)  # Increased
        inner_width = int(bolt_width * 1.4)    # Increased
        inner_rect = pg.Rect(surf_center_x - inner_length//2, surf_center_y - inner_width//2,
                           inner_length, inner_width)
        pg.draw.ellipse(bolt_surface, (100, 220, 255, 180), inner_rect)
        
        # Bright energy center - bigger
        core_length = int(bolt_length * 0.6)   # Increased
        core_width = max(8, int(bolt_width * 0.8))  # Increased minimum from 3 to 8 (2.5x+)
        core_rect = pg.Rect(surf_center_x - core_length//2, surf_center_y - core_width//2,
                          core_length, core_width)
        pg.draw.ellipse(bolt_surface, (255, 255, 255, 220), core_rect)
        
        # Add energy sparks at front and back - bigger
        # Front spark - bigger
        front_x = surf_center_x + outer_length//2
        pg.draw.circle(bolt_surface, (255, 255, 255, 255), (front_x, surf_center_y), 8)  # Increased from 3 (2.5x+)
        
        # Trailing energy wisps - bigger
        for i in range(4):  # More wisps
            wisp_x = surf_center_x - outer_length//2 - i * 10  # Spaced further apart (2.5x from 4)
            wisp_alpha = 180 - i * 35  # Brighter
            if wisp_alpha > 0:
                pg.draw.circle(bolt_surface, (100, 200, 255, wisp_alpha), (wisp_x, surf_center_y), 5)  # Bigger wisps (2.5x from 2)
        
        # Rotate the energy bolt to match angle
        rotated_bolt = pg.transform.rotate(bolt_surface, -self.angle)
        
        # Position and render
        bolt_rect = rotated_bolt.get_rect(center=(int(x), int(y)))
        screen.blit(rotated_bolt, bolt_rect, special_flags=pg.BLEND_ADD)
    ...
    def render_neon_bullet(self, screen: pg.Surface, x: float, y: float):
        ...
        # Cyberpunk neon colors (cyan theme)
    ...
    def _draw_laser_particles(self, screen: pg.Surface, tip_x: float, tip_y: float, angle_rad: float):
        """Draw particle effects at the laser tip."""
        import math
        import random
        ...
        # Create streaming particle trail behind the tip - longer trail
        for i in range(15):  # Increased from 8 for longer trail
            trail_dist = i * 5  # Increased spacing
            trail_x = tip_x - cos_angle * trail_dist
            trail_y = tip_y - sin_angle * trail_dist
            
            # Random offset for natural look
            offset_x = random.uniform(-4, 4)  # Increased spread
            offset_y = random.uniform(-4, 4)
            
            particle_size = max(1, 6 - i // 3)  # Larger particles
            alpha = 255 - (i * 15)  # Slower fade
            color = (255, min(255, 200 + i * 5), min(255, 150 + i * 8))
            
            pg.draw.circle(screen, color, (int(trail_x + offset_x), int(trail_y + offset_y)), particle_size)
        
        # Create multiple particle rings around the tip - larger rings
        for ring in range(5):  # One more ring
            ring_radius = (ring + 1) * 8  # Increased from 4
            particles = 12 + ring * 4  # More particles per ring
            
            for i in range(particles):
                # Random particle position around the tip
                particle_angle = (i / particles) * 2 * math.pi + random.uniform(-0.5, 0.5)
                particle_dist = ring_radius + random.uniform(-5, 5)  # Increased spread
                
                particle_x = tip_x + math.cos(particle_angle) * particle_dist
                particle_y = tip_y + math.sin(particle_angle) * particle_dist
                
                # Particle colors - brighter in center, dimmer outside
                if ring == 0:
                    color = (255, 255, 255)
                    size = 5  # Larger center particles
                elif ring == 1:
                    color = (220, 245, 255)
                    size = 4
                elif ring == 2:
                ...
                # Add some randomness to particle visibility
                if random.random() > 0.2:  # 80% chance to show particle (was 70%)
                    pg.draw.circle(screen, color, (int(particle_x), int(particle_y)), size)
        
        # Add a bright center flash with pulsing effect - much larger
        flash_size = 8 + int(random.uniform(-2, 2))  # Larger flash
        pg.draw.circle(screen, (255, 255, 255), (int(tip_x), int(tip_y)), flash_size)
        pg.draw.circle(screen, (200, 240, 255), (int(tip_x), int(tip_y)), flash_size + 3, 2)  # Thicker ring
        pg.draw.circle(screen, (150, 200, 255), (int(tip_x), int(tip_y)), flash_size + 6, 2)  # Thicker outer ring
    
    def _draw_energy_crackling(self, screen: pg.Surface, back_x: float, back_y: float, front_x: float, front_y: float, beam_width: float):
        """Draw energy crackling effects along the beam."""
        import math
        import random
        
        beam_length = math.sqrt((front_x - back_x)**2 + (front_y - back_y)**2)
        
        # Draw several energy crackles along the beam - more for longer beam
        num_crackles = max(int(beam_length / 15), 4)  # More crackles for longer beam
        ...
        for i in range(num_crackles):
            ...
            # Random crackling offset - larger for bigger beam
            offset_dist = random.uniform(beam_width * 0.8, beam_width * 2.5)  # Increased range
            offset_angle = random.uniform(0, 2 * math.pi)
            
            crack_x = base_x + math.cos(offset_angle) * offset_dist
            crack_y = base_y + math.sin(offset_angle) * offset_dist
            
            # Draw crackling line back to beam - thicker lines
            color = (180, 220, 255) if random.random() > 0.5 else (255, 255, 255)
            line_width = 2 if random.random() > 0.7 else 1  # Some thicker crackle lines
            pg.draw.line(screen, color, (int(base_x), int(base_y)), (int(crack_x), int(crack_y)), line_width)
...
class BulletManager:
    ...
    def create_bullet(self, x: float, y: float, angle: float, bullet_type: BulletType = BulletType.PLAYER, damage: int = None, speed: float = 800, size_multiplier: float = 1.0, color: tuple = None, penetration: int = 1, shape: str = None, range_limit: float = None, weapon_type: str = None):
        ...
        return Bullet(x, y, angle, bullet_type, speed=speed, damage=damage, size_multiplier=size_multiplier, color=color, penetration=penetration, shape=shape, range_limit=range_limit, weapon_type=weapon_type)
    
    def shoot(self, x: float, y: float, angle: float, current_time: float, bullet_type: BulletType = BulletType.PLAYER, damage: int = None, speed: float = 800, size_multiplier: float = 1.0, color: tuple = None, penetration: int = 1, shape: str = None, range_limit: float = None, weapon_type: str = None) -> bool:
        """Create a new bullet if fire rate allows. Returns True if bullet was fired."""
        if self.can_shoot(current_time):
            bullet = Bullet(x, y, angle, bullet_type, speed=speed, damage=damage, size_multiplier=size_multiplier, color=color, penetration=penetration, shape=shape, range_limit=range_limit, weapon_type=weapon_type)
